...
We recommend using the `surveydown::create_survey()` function to start with a template:

```{r}
#| eval: false

surveydown::create_survey("path/to/folder")
```

replacing `"path/to/folder"` with the path where you want to create your template surveydown project. You can also provide a `template` argument to specify which template to use (for now there is only one template: `"simple"`).

> Every survey created with {surveydown} should be in it's own separate project folder.
