...
- New feature (#247): `sd_values()` and `sd_value()` as new approach of accessing question values, replacing the Shiny default `input$`. `sd_values()` and `sd_value()` are able to restore user inputs from db after refreshing the page. They also accept either quoted question IDs or unquoted, so that either `sd_value(fruit)` or `sd_value("fruit")` is fine. They also support multiple parameters, which return into a vector of values. For example, `sd_value(fruit, vegetable)` returns `c("apple", "lettuce")`. They support auto-conversion to numeric under proper conditions. For example, if the output is `"40"` or `c("40", "50")`, it'll be auto-converted to `40` and `c(40, 50)`. In short, `sd_value(something)` is equivalent to `as.numeric(input$something)`. sd_value()` is the alias of `sd_values()`, so they function the same.
